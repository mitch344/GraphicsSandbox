...
Texture2D g_DiffuseTexture : register(t4);
Texture2D g_SpecularTexture : register(t5);
Texture2D g_AmbientTexture : register(t6);
Texture2D g_EmissiveTexture : register(t7);
Texture2D g_NormalTexture : register(t8);
Texture2D g_BumpTexture : register(t9);
Texture2D g_DisplacementTexture : register(t10);
Texture2D g_StencilDecaTexture : register(t11);
Texture2D g_ReflectionTexture : register(t12);
Texture2D g_ShininessTexture : register(t13);
Texture2D g_AlphaTexture : register(t14);
...
float4 main(SHADER_INPUT input) : SV_TARGET
{
    ...
    //Load Texture Maps
    float3 diffuseMap = DiffuseMap ? g_DiffuseTexture.Sample(g_Sampler, input.texCoord).rgb : float3(1, 1, 1);
    ...
    diffuseColor = DiffuseMap ? diffuseColor * diffuseMap : diffuseColor;
    specularColor = SpecularMap ? specularColor * specularMap : specularColor;
    ambientColor = AmbientMap ? ambientColor * ambientMap : ambientColor;
